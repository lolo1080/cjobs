...
<td>
    <form style="padding:2px">
        ...
        Text: <input type="text" name="text" value="<?php =time()?>">
        <i>Type "error" to show debug capabilities.</i><br>
        <table>
        <?php foreach (array('', 'get', 'post') as $method) {?>
            <tr>
            <td><?php =$method? strtoupper($method) : 'AUTO'?>:</td>
            <td>
                <?php foreach (array('', 'xml', 'script', 'form') as $loader) {?>
                    <input type="button" 
                        onclick="request('<?php =$loader?>', '<?=$method?>', this.form.text.value)" 
                        value="<?php =$loader? $loader : 'AUTO'?>"
                    >
                <?php }?>
            </td>
            </tr>
...
<td>
    <form style="padding:2px" enctype="multipart/form-data">
        ...
        Text: <input type="text" name="text" value="<?php =microtime()?>">
        <i>Type "error" to show debug capabilities.</i><br>
        File: <input type="file" name="file"><br>
        <table>
        <?php foreach (array('', 'get', 'post') as $method) {?php >
            <tr>
            <td><?php =$method? strtoupper($method) : 'AUTO'?>:</td>
            <td>
                <?php foreach (array('', 'xml', 'script', 'form') as $loader) {?>
