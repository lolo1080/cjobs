...
function create_values($email,$company,$name,$phone,$fax,$site,$address1,$address2,$country_id,$city,$state,$zipcode,
	$promotioncode,$ssn)
{
 global $smarty,$text_info,$usersettings;
	...
	$smarty->assign("reginfo",$text_info["i_pub_signup"]);
}
...
function try_register()
{
 global $Error_messages, $db_tables;
	...
	$vallist = array($pass,$repass,$name,$site);
	$errlist = array($Error_messages["no_pass"],$Error_messages["no_repass"],$Error_messages["no_name"],$Error_messages["no_site"]);
	...
	if (find_pub_email_in_db($_SESSION["sess_confirm_email"])) $my_error .= $Error_messages["find_email_in_db"];
	...
	if ($my_error == "") {
		...
		//add data
		add_data($_SESSION["sess_confirm_email"],$pass,$company,$name,$phone,$fax,$site,$address1,$address2,$country,$city,$state,
			$zipcode,$promotioncode,$ssn);
	}
	else {//else - try again
...
function singup_send_email($name,$username,$pass,$uid,$email)
{
 global $parse_values;
	...
	$parse_values["{*login_url_txt*}"]				= $_SESSION["globsettings"]["site_url"].'publishers/';
	...
	//Sign Up welcome
	$subj	= get_mailsubject("sign_up_welcome_pub");
	$htmlmessage = get_email_file("sign_up_welcome_pub","html");
	$textmessage = get_email_file("sign_up_welcome_pub","txt");
	$attach_files= get_mail_attach("sign_up_welcome_pub");
			...
			$zipcode,$promotioncode,$ssn)
{
 global $db_tables, $SLINE;
	//Start balaance
	$amount = $_SESSION["globsettings"]["pub_start_balance"];
	//Member approved:
	$m_a = (int)!$_SESSION["globsettings"]["member_approved"];
	//Insert new member data
	mysql_query("INSERT INTO ".$db_tables["users_publisher"]." VALUES(NULL,'$email','$pass','$company','$name',".
		"'$phone','$fax','$site','$address1','$address2','$country_id','$city','$state','$zipcode','$promotioncode',".
		"'$ssn','$amount',NOW(),NULL,$m_a,1,0,0)") or query_die(__FILE__,__LINE__,mysql_error());
	$uid = mysql_insert_id();
	//Send event
	$event_array = array("event"=>"insert", "source"=>"publishers", "table"=>"users_publisher", "ad_id"=>0);
	event_handler($event_array);
	//Insert member personal configuration
	mysql_query("INSERT INTO ".$db_tables["users_publisher_settings"]." VALUES('$uid','1')")	or query_die(__FILE__,__LINE__,mysql_error());
	//Send e-mail
	singup_send_email($name,$email,$pass,$uid,$email);
	header("Location: pub_registration_success.php?$SLINE"); exit;
}
...
function try_confirm_register()
{
 global $Error_messages,$db_tables,$SLINE;
	...
	if (find_pub_email_in_db($email)) $my_error .= $Error_messages["find_email_in_db"];
	...
	if ($my_error == "") {
		...
		//Send e-mail
		signup_send_confirm_email($email,$emailkey,"pub");
		header("Location: pub_registration_success.php?$SLINE"); exit;
	}
	//else - try again
	smarty_create_message("error","abort.gif",$my_error);
