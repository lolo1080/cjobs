...
if ($my_error != "") smarty_create_message("error","info.gif",$my_error);
	...
	$qr_res = mysql_query("DELETE FROM ".$db_tables["xml_feeds_data_temp"]." WHERE feed_id='{$feed_id}'") or query_die(__FILE__,__LINE__,mysql_error());
	$qr_res = mysql_query("INSERT INTO ".$db_tables["xml_feeds_data_temp"]." VALUES(NULL,'{$feed_id}','$cat_id','".addslashes($url)."','$config_id')") or query_die(__FILE__,__LINE__,mysql_error());
