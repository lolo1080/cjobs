...
require_once 'xml_parser/class.xml2array.corrected.php';
require_once 'xml_parser/php45fix.php';
...
$vallist = array($feed_id,$config_id,$url);
$errlist = array($Error_messages["no_feed_id"],$Error_messages["no_config_id"],$Error_messages["no_url"]);
...
if ($my_error != "") smarty_create_message("error","info.gif",$my_error);
	...
	$qr_res = mysql_query("DELETE FROM ".$db_tables["xml2_feeds_data_temp"]." WHERE feed_id='{$feed_id}'") or query_die(__FILE__,__LINE__,mysql_error());
	$qr_res = mysql_query("INSERT INTO ".$db_tables["xml2_feeds_data_temp"]." VALUES(NULL,'{$feed_id}','".addslashes($url)."','$config_id')") or query_die(__FILE__,__LINE__,mysql_error());
	...
	if (!isset($Configuration)) {
		$check_result = array("cat_name"=>'no (xml2 type)', "config_name"=>$feed_row["config_name"], "checked_url"=>$url,
		...
		foreach ($Configuration["fields"] as $k=>$v)
		{
			if ($Configuration["fields"][$k]["name"] == "description") {
				...
				{
					$Configuration["fields"][$k]["data"][$k1] = substr(convert_html_to_text(trim( $v1 )),0,157);
...
$smarty->display('page_feeds_xml_check.tpl');
?>