<?
function create_values($uid_pub,$email,$company,$name,$balance)
{
...
function update_user_info($uid_pub,$balance)
{
 global $db_tables, $SLINE;
	mysql_query("UPDATE ".$db_tables["users_publisher"]." SET balance='$balance' WHERE uid_pub='$uid_pub'") or query_die(__FILE__,__LINE__,mysql_error());
	//Send event
	$event_array = array("event"=>"update", "source"=>"publishers", "table"=>"users_publisher", "ad_id"=>0);
	event_handler($event_array);
	header("Location: publishers_work.php?action=edit&uid_pub={$uid_pub}&{$SLINE}"); exit;
}
...
function try_save($uid_pub)
{
 global $smarty,$Error_messages,$text_info,$db_tables;
	...
	if ($my_error == "") {
		...
		//Update data
		update_user_info($uid_pub,$balance);
	}
	else { //else - try again
		smarty_create_message("error","abort.gif",$my_error);
		//Create form and buttons
		create_values($uid_pub,$email,$company,$name,$balance);
