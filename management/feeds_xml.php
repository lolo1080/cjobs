...
//feeds work page
$smarty->assign("curpage","feeds_xml");
...
$Configuration = array(
	...
	"configuration_list"=> get_configuration_list("WHERE feed_format='xml1'"),
	"categories_list" 	=> get_categories_list()
);

//* * * Data from DB data * * *
$qr_res = mysql_query("SELECT * FROM ".$db_tables["xml_feeds_data"]." WHERE feed_id='$feed_id' ORDER BY cat_id") or query_die(__FILE__,__LINE__,mysql_error());
...
//			if ($v[$j] == '') $my_error .= str_replace("{*Num*}",$i.':'.$j,$Error_messages["no_category_url"]);
	...
	if ($my_error == '') {
		mysql_query("DELETE FROM ".$db_tables["xml_feeds_data"]." WHERE feed_id='{$feed_id}'") or query_die(__FILE__,__LINE__,mysql_error());
		foreach ($category_url as $k=>$v)
		{
			for ($j=0; $j<count($v); $j++)
			{
				if ($v[$j] == '') continue;
				mysql_query("INSERT INTO ".$db_tables["xml_feeds_data"]." VALUES(NULL,'".$feed_id."','".$k."','".$v[$j]."','".$category_id[$k][$j]."')") or query_die(__FILE__,__LINE__,mysql_error());
			}
		}
