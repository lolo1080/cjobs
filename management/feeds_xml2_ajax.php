...
function insert_category_data($mode_data,&$return_category)
{
 global $db_tables;
	foreach ($return_category as $cat_id=>$keywords)
	{
		mysql_query("INSERT INTO ".$db_tables["xml2_feeds_category_keywords"]." VALUES(NULL,'".$mode_data."','".$cat_id."','".$keywords."')") or query_die(__FILE__,__LINE__,mysql_error());
	}
}
...
try {
	...
	switch ($_POST['cmd']) {
		...
		case 'save_configurtion':
			...
			$return_category = get_post_value2("return_category",array());
			addslashes_and_html_array_notrelated($return_category);
			//Type
			...
			check_return_phpdata_before('category');
			//URL
			...
			mysql_query("UPDATE ".$db_tables["xml_feeds_configuration"]." SET config_name='".$name."',config_url='".$url."',title_status='".
				...
				PrepAVal($return_phpcodetext_after,'salary')."',category_status='".

				PrepAVal($return_status,'category')."',category_field='".
				PrepAVal($return_box,'category')."',category_run_phpcode_before='".
				PrepAVal($run_phpcode_before,'category')."',category_phpcode_before='".
				PrepAVal($phpcodetext_before,'category')."',category_phpcode='".
				PrepAVal($return_parsed_data,'category')."',category_run_phpcode_after='".
				PrepAVal($return_phpcodetext_after,'category')."',category_phpcode_after='".
				PrepAVal($return_phpcodetext_after,'category')."' ".

				"WHERE config_id='".$mode_data."'") or query_die(__FILE__,__LINE__,mysql_error());

				//Update category info
				$qr_res = mysql_query("DELETE FROM ".$db_tables["xml2_feeds_category_keywords"]." WHERE config_id='$mode_data'") or query_die(__FILE__,__LINE__,mysql_error());
				insert_category_data($mode_data,$return_category);
			...
			mysql_query("INSERT INTO ".$db_tables["xml_feeds_configuration"]." VALUES(NULL,'".$name."','".$url."','".
				...
				PrepAVal($return_status,'category')."','".
				PrepAVal($return_box,'category')."','".
				PrepAVal($run_phpcode_before,'category')."','".
				PrepAVal($phpcodetext_before,'category')."','".
				PrepAVal($return_parsed_data,'category')."','".
				PrepAVal($return_phpcodetext_after,'category')."','".
				PrepAVal($return_phpcodetext_after,'category')."',".

				"NOW(),'xml2')") or query_die(__FILE__,__LINE__,mysql_error());

				$mode_data = mysql_insert_id();
				insert_category_data($mode_data,$return_category);
		...
		case 'edit':
			...
			$Result['config_data'] = get_configuration_by_id_xml2($config_id);
			//Return data
			//echo json_encode($Result); exit;
			echo php2js($Result); exit;
		case 'delete':
			...
			$qr_res = mysql_query("DELETE FROM ".$db_tables["xml2_feeds_category_keywords"]." WHERE config_id='$config_id'") or query_die(__FILE__,__LINE__,mysql_error());
			$Result['status'] = 'ok';
			//Return data
			//echo json_encode($Result); exit;
			echo php2js($Result); exit;
