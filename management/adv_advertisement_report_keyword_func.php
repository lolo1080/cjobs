...
function get_period_info($stattype)
{
 global $smarty,$db_tables,$usersettings,$Error_messages,$my_error,$limitation,$having_limitation,
	...
	$qr_res = mysql_query("CREATE TEMPORARY TABLE IF NOT EXISTS stats_report_keyword_ads_tmp (".
	...
	mysql_query("TRUNCATE stats_report_keyword_ads_tmp") or query_die(__FILE__,__LINE__,mysql_error());

	// * * Check cache * * //
	$cache_params_array = array(
		"user"					=> $_SESSION["sess_user"],
		"userid"				=> $query_uid_adv,
		"stats_query"		=> "adv_stats_report_keyword_ads",
		"stats_type"		=> $stattype,
		"stats_type_a"	=> $a,
		"stats_type_b"	=> $b,
		"params_list"		=> array($ad_id),
		"table_name"		=> "stats_report_keyword_ads_tmp"
	);
	...
	if (!read_stats_cache($cache_params_array)) {
		...
		$qr_res = mysql_query("SELECT count(*) as clicks, sum(cost) as costs, DATE_FORMAT(actiontime, '%Y-%m-%d') as actiondate ".
			...
			"WHERE uid_adv='{$query_uid_adv}' and ad_id='$ad_id' and $cur_cond_line ".
		...
		$qr_res = mysql_query("SELECT count(*) as maybe_adviews, sum(page) as maybe_pages, DATE_FORMAT(actiontime, '%Y-%m-%d') as actiondate ".
			"FROM ".$db_tables["stats_adv_maybe_pageview_keywords"]." ".
			"WHERE uid_adv='{$query_uid_adv}' and ad_id='$ad_id' and $cur_cond_line ".
			"GROUP BY actiondate")	or query_die(__FILE__,__LINE__,mysql_error());
		while ($myrow = mysql_fetch_array($qr_res))
		{
		...
		if (isset($report_date["type"])) {
			...
			if (mysql_num_rows($qr_res) > 0) {
				...
				while ($i_cur_date <= $i_end_date)
				{
					...
					mysql_query("INSERT INTO stats_report_keyword_ads_tmp(actiondate,clicks,adviews,ctr,avg_cpc,cost,avg_pos) ".
	...
	$DataBody = array();
	$page_count = get_page_count("SELECT count(*) as num FROM stats_report_keyword_ads_tmp ".$limitation,$row_count);
//	$page_count = ceil($num/$row_count);
	...
	$num  = 0;
	$qr_res = mysql_query("SELECT *,actiondate as datesort,".format_sql_date("actiondate")." as actiondate FROM stats_report_keyword_ads_tmp  ".
...
/*
		$qr_res = mysql_query("SELECT sum(clicks) as clicks, sum(adviews) as adviews, sum(ctr) as ctr, sum(avg_cpc) as avg_cpc,".
			"sum(cost) as cost, sum(avg_pos) as avg_pos FROM stats_report_keyword_ads_tmp ".
			" $limitation LIMIT $from_count, $row_count") 
				or query_die(__FILE__,__LINE__,mysql_error());
*/
		$qr_res = mysql_query("SELECT sum(clicks) as clicks, sum(adviews) as adviews, sum(cost) as cost, avg(avg_pos) as avg_pos FROM stats_report_keyword_ads_tmp ".
			" $limitation ") 
				or query_die(__FILE__,__LINE__,mysql_error());
		while ($myrow = mysql_fetch_array($qr_res))
		{
			$DataBody[$num] = array(
				...
				array("tdw"=>"75", "tdclass"=>"tbl_td_data_total","tdalign"=>"","data"=>'&nbsp;'.sprintf("%01.2f", (($limitation == "") ? get_total_avg_pos($cond_line,$query_uid_adv,$ad_id) : $myrow["avg_pos"])) )
			);
			$num++;
		}
	}
...
function get_total_avg_pos($cond_line,$query_uid_adv,$ad_id)
{
 global $db_tables;
	//Avg Pos
