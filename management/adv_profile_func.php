...
function update_data($pass,$company,$name,$phone,$fax,$site,$address1,$address2,$country,$city,$state,$zipcode)
{
 global $db_tables;
	mysql_query("UPDATE ".$db_tables["users_advertiser"]." SET pass='$pass',company='$company',name='$name',".
				"phone='$phone',fax='$fax',site='$site',address1='$address1',address2='$address2',".
				"country_id='$country',city='$city',state='$state',zipcode='$zipcode'".
				" WHERE uid_adv='".$_SESSION["sess_userid"]."'") or query_die(__FILE__,__LINE__,mysql_error());
	//Send event
...
function try_change()
{
 global $Error_messages, $db_tables;
	...
	$vallist = array($pass,$company,$name,$phone);
	$errlist = array($Error_messages["no_pass"],$Error_messages["no_company"],$Error_messages["no_name"],
			$Error_messages["no_phone"]);
	...
	if ($my_error == "") {
		...
		//Update data
		update_data($pass,$company,$name,$phone,$fax,$site,$address1,$address2,$country,$city,$state,$zipcode);
	}
	else {//else - try again
		smarty_create_message("error","abort.gif",$my_error);
	}
	$qr_res = mysql_query("SELECT *,".format_sql_date("regdate")." as regdate FROM ".$db_tables["users_advertiser"]." WHERE uid_adv='".$_SESSION["sess_userid"]."'") or query_die(__FILE__,__LINE__,mysql_error());
	$myrow = mysql_fetch_array($qr_res);
	create_values($myrow["email"],$pass,$company,$name,$phone,$fax,$site,$address1,$address2,$country,$city,$state,$zipcode,
		$myrow["promotioncode"],$myrow["regdate"]);
}
?>