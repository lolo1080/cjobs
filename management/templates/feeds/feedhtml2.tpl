...
function get_data_from_html_feed(url)
{
	hide_feed_error();
	$.ajax({
		type		: 'POST',
		url			: 'feeds_html2_ajax.php',
		data		: { 'cmd': 'get', 'url': url },
		cache		: false,
		success	: function(res) {
			var obj = jQuery.parseJSON(res);
			$("#html_rawdata").val(obj.html_rawdata);
			if (obj.status == 'error') {
				show_feed_error(obj.errormsg);
				if ($("#debug_mode:checked")) $("#html_rawdata").value(obj.html_rawdata);
				return;
			}
			$("#div_html_parsedfields").html(obj.html_parsedfields);
			$("#div_html_parsedfields_help").show();
			$("#html_tree").treeview({
...
/*Get selected data from URL functions*/
function get_field_data(el,type,nm)
{
	if (nm == '') return;
	$("#selected_field_template").html('<input type="text" name="selected_field_template_text" id="selected_field_template_text" value="{*'+type+'*'+nm+'*'+type+'*}" style="width:500px;" readonly /> Replace variable part with "?" and copy to HTML field');
}

/*
function get_field_data(el,nm)
{
	var selval = $(el);
...
/*Get reg expr result on feed*/
function get_data_from_html_feed_parsed_by_regexpr(url, regular_expression)
{
	hide_feed_error();
	$.ajax({
		type		: 'POST',
		url			: 'feeds_html2_ajax.php',
		data		: { 'cmd': 'regular_expression', 'url': url, 'regular_expression': regular_expression },
		cache		: false,
		success	: function(res) {
			var obj = jQuery.parseJSON(res);
			$("#html_regexpr_result").html('<textarea style="width:900px;height:200px;">'+obj.html_regexpr_result+'</textarea>');
			if (obj.status == 'error') {
				show_feed_error(obj.errormsg);
				return;
			}
...
function save_configuration()
{
	...
	for (var i=0; i<field_names.length; i++)
	{
		...
		if (val == '1') {
			...
			val = $("#"+id).val();
			if (val == "") { alert('Please enter "'+field_titles[i]+'" HTML field'); return; }
			else data += "&"+idd+"="+val;
		}
...
function edit_configuration(id)
{
	...
	$.ajax({
		...
		success	: function(res) {
			...
			obj = resobj.config_data;
			//Fill fields
			$('#mode').val('edit');
			$('#mode_data').val(id);
			$('#url').val(obj.config_url);
			$('#html_parse_regular_expression').val(obj.html_parse_regular_expression);
...
function create_new_configuration()
{
	...
	$('#mode_data').val('');
	$('#url').val('');
	$('#html_parse_regular_expression').val('');
	$('#name').val('');
	{/literal}
	{section name=i loop=$Configuration.fields}
		$('#return_status1\\[{$Configuration.fields[i].name}\\]').attr('checked', 'checked');
...
$('.actionbutton_add').live('click', function() {
	...
	$clonedRow.insertAfter('#data_items_tbl_'+$clonedRow_num+' tbody>tr.data_items_tbl_tr:last');
...
$('.actionbutton_delete').live('click', function() {
	...
	if ($('#data_items_tbl_'+$clonedRow_num+' tbody>tr.data_items_tbl_tr').length <= 1) return;
...
function check_data_parsing(btn)
{
	...
	window.open('feeds_html2_check.php?cat_id='+cat_id+'&feed_id='+feed_id+'&config_id='+config_id+'&url='+$.URLEncode(url));
}
...
$(document).ready(function() {
	$("#url_send").bind('click', function(e) {
		...
		else get_data_from_html_feed(url);
	});
	$("#regular_expression_check").bind('click', function(e) {
		var url = $("#url").val();
		if (url == "") { alert('{/literal}{$Configuration.err_no_url|strip_tags}{literal}'); return; }
		var regular_expression = $("#html_parse_regular_expression").val();
		if (regular_expression == "") { alert('Please, enter Regular expression');  return; }
		get_data_from_html_feed_parsed_by_regexpr(url, regular_expression);
	});
	lock_first_del_btn();
});
{/literal}
...
<form name="feed_data" action="feeds_html.php" method="POST">
	...
	<tr>
		<td class="tbl_td_head" style="height:30px;padding-top:5px;">Category name</td>
		<td class="tbl_td_head" style="height:30px;padding-top:5px;">Parser information</td>
	</tr>
	{section name=i loop=$Configuration.categories_list}
	<tr>
		<td>{$Configuration.categories_list[i].cat_name}:</td>
		<td>
			<table width="100%" id="data_items_tbl_{$Configuration.categories_list[i].cat_id}" class="data_items_table" cellpadding="2" cellspacing="2">
				{if $Configuration.acttype == 'new'}

					<tr id="datarow_{$Configuration.categories_list[i].cat_id}" class="data_items_tbl_tr">
						<td>
							URL:<input type="text" name="category_url[{$Configuration.categories_list[i].cat_id}][]" id="category_url[{$Configuration.categories_list[i].cat_id}][]" class="category_url" value="" style="width:300px;" />
							Config:<select name="category_id[{$Configuration.categories_list[i].cat_id}][]" id="category_id[{$Configuration.categories_list[i].cat_id}][]" class="category_config" style="width:140px;">
							...
							<input type="button" value="check parsing" onClick="check_data_parsing(this);" rel="{$Configuration.categories_list[i].cat_id}" />
						</td>
				...
				{else}
					...
					{assign var="key" value=$Configuration.categories_list[i].cat_id}
					{foreach name=category_url_item key=key1 item=item1 from=$Configuration.save.category_url[$key]}
						{foreach name=category_url_item2 key=key2 item=item2 from=$item1}
							<tr id="datarow_{$key}" class="data_items_tbl_tr">
								<td>
{*									URL:<input type="text" name="category_url[{$key}][{$key1}]" id="category_url[{$key}][{$key1}]" class="category_url" value="{$item2}" style="width:300px;" />*}
{*									Config:<select name="category_id[{$key}][{$key1}]" id="category_id[{$key}][{$key1}]" class="category_config" style="width:140px;">*}
									URL:<input type="text" name="category_url[{$key}][]" id="category_url[{$key}][]" class="category_url" value="{$item2}" style="width:300px;" />
									Config:<select name="category_id[{$key}][]" id="category_id[{$key}][]" class="category_config" style="width:140px;">
										{section name=j loop=$Configuration.configuration_list}
											<option value="{$Configuration.configuration_list[j].config_id}"{if ($Configuration.save.category_id[$key][$key1]==$Configuration.configuration_list[j].config_id)} selected{/if}>{$Configuration.configuration_list[j].config_name}</option>
										{/section}
									</select>
									<input type="button" value="check parsing" onClick="check_data_parsing(this);" rel="{$Configuration.categories_list[i].cat_id}" />
								</td>
...
<div style="font-family: arial; font-size: 12px;">
 <div style="padding-left:20px;">
	...
	After click on "Check" button parsing process may take some time and you may see "Stop script" popup window. Do not stop script.<br />
	<div id="error_box" style="display:none;"><br /></div>
	<div>
		<textarea class="data" name="html_rawdata" id="html_rawdata" style="width:900px;height:200px;">{$Configuration.html_rawdata}</textarea>
	</div>
	<div id="div_html_parsedfields">
		{$Configuration.div_html_parsedfields}
	</div>
	<div id="div_html_parsedfields_help" style="padding-bottom:5px; display:none;">
		{literal}
		{*c*}[...][...]{*c*} - copy tag content. Copy tag and content between open and close tags <br />
		{*t*}[...][...]{*t*} - copy tag. Copy content from open tag <br />
		{*cs*}[...][...]{*cs*} - copy slashed (" to \" and ' to \') tag content.<br />
		{*ts*}[...][...]{*ts*} - copy slashed (" to \" and ' to \') tag.<br />
		1) You should use {*cs*} and {*ts*} if you want to use this value inside "PHP parsed HTML field"<br />
		For example, $resdata = '{*cs*[html][0][body][div][30]*cs*}';<br />
		2) You should remove slashes by stripslashes() function when return data from "PHP parsed HTML field"<br />
		For example, $result = stripslashes($resdata);
		{/literal}
	</div>
	<div id="div_html_Rres">
	</div>
<!--
	<div id="div_html_parse_regular_expression">
		Regular expression:<input type="text" name="html_parse_regular_expression" id="html_parse_regular_expression" value="{$Configuration.html_parse_regular_expression}" style="width:726px;" />
		<input type="button" name="regular_expression_check" id="regular_expression_check" value="Check" />
	</div>
-->
...
<!--
	<div id="html_regexpr_result"></div>
	<div style="float:left;">
		<span>{$Configuration.selectedfieldtitle}:</span><br />
		<span id="selected_field_template"></span>
	</div>
...
-->
	...
	{section name=i loop=$Configuration.fields}
		<tr>
			...
			<td>
					<input type="radio" name="return_status[{$Configuration.fields[i].name}]" id="return_status1[{$Configuration.fields[i].name}]" value="1" checked />Return HTML field:
					<input type="text" name="return_box[{$Configuration.fields[i].name}]" id="return_box[{$Configuration.fields[i].name}]" style="width:350px;" /><br />
						<small>
							<ul style="padding:0; padding-left:35px; margin:0;">
								<li>Use regular expression {ldelim}*MainRegExpr[X][?]*{rdelim}&nbsp;&nbsp;For example: [ {ldelim}*MainRegExpr[2][?]*{rdelim} ]</li>
							</ul>
						</small>
					<input type="radio" name="return_status[{$Configuration.fields[i].name}]" id="return_status2[{$Configuration.fields[i].name}]" value="2" />Return PHP parsed HTML field
					<div>
						<!--
						...
						<small>
							<ul style="padding:0; padding-left:35px; margin:0;">
								...
								<li>You can use next constructions '{ldelim}*&lt;HTML field here&gt;*{rdelim}'</li>
								<li>To return calculated string just put to $result value.<br />&nbsp;&nbsp;For example: [ $result = trim( {ldelim}*MainRegExpr[1][?]*{rdelim} ); ]</li>
								<li>To get content from external site use next function: string get_content_from_site (string url, string method), where method can be "get" or "post"<br />&nbsp;&nbsp;For example: [ $content = get_content_from_site('{*MainRegExpr[2][?]*}','get'); ]</li>
								<li>To get url from tag use next function: string get_url_from_tag (string tag)<br />&nbsp;&nbsp;For example: [ $joburl = .get_url_from_tag('{*ts*[html][0][body][4][table][14][tr][1][td][1][table][4][tr][2][td][4][div][?][h2][2][a][1]*ts*}'; ]</li>
								<li>Do not override next values: $i, $k, $v, $feed_row, $log_actions, $FeedName, $url, $MainRegExpr, $Result</li>
							</ul>
						</small>
