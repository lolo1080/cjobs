...
//Check first entry
check_first_entry("publishers",array());
...
if (isset($_SESSION["sess_xmlfeed_subm_type"])) unset($_SESSION["sess_xmlfeed_subm_type"]);
...
switch ($action) {
	case "chstatus":
		for ($i=0; $i<count($elist); $i++) {
			...
			if (check_int($elist[$i])) mysql_query("UPDATE ".$db_tables["users_publisher"]." SET isenable=not isenable WHERE uid_pub='".$elist[$i]."'") or query_die(__FILE__,__LINE__,mysql_error());
		}
		//Send event
	...
	case "delete":
		for ($i=0; $i<count($elist); $i++) {
			$elist[$i] = data_addslashes(trim($elist[$i]));
			if (check_int($elist[$i])) mysql_query("UPDATE ".$db_tables["users_publisher"]." SET isdeleted='1',deldate=CURDATE() WHERE uid_pub='".$elist[$i]."'") or query_die(__FILE__,__LINE__,mysql_error());
		}
		//Send event
		$event_array = array("event"=>"update", "source"=>"publishers", "table"=>"users_publisher", "ad_id"=>0);
		event_handler($event_array);
...
$smarty->assign("DataHead", array(
	...
	array("tdw"=>"75", "tdclass"=>"tbl_td_head","data"=>sort_link("publishers.php","regdate",$text_info["th_regdate"])),
	array("tdw"=>"200","tdclass"=>"tbl_td_head","data"=>sort_link("publishers.php","email",$text_info["th_email"])),
	array("tdw"=>"120","tdclass"=>"tbl_td_head","data"=>sort_link("publishers.php","name",$text_info["th_name"])),
	array("tdw"=>"85", "tdclass"=>"tbl_td_head","data"=>sort_link("publishers.php","isenable",$text_info["th_status"])),
	array("tdw"=>"85", "tdclass"=>"tbl_td_head","data"=>$text_info["th_all_status"]),
	array("tdw"=>"85", "tdclass"=>"tbl_td_head","data"=>$text_info["th_xmlfeed_status"]),
...
smarty_create_filter($text_info["filter_help"],$text_info["filter"],$text_info["removefilter"],"publishers.php",3,$FilterElements);
...
$DataBody = array();
$dev = "&nbsp;/&nbsp;";
$page_count = get_page_count("SELECT count(*) as num FROM ".$db_tables["users_publisher"]." WHERE isdeleted=0".$limitation,$row_count);
if ($start > ($page_count-1)) $start = $page_count-1;
if ($start < 0) $start = 0;
$from_count = $start*$row_count;
$qr_res = mysql_query("SELECT uid_pub,email,name,regdate as datesort,".format_sql_date("regdate")." as regdate,isenable,isconfirmed,isxmlfeed_enable ".
		"FROM ".$db_tables["users_publisher"]." ".
...
{
	$str_link_edit = '<a href="publishers_work.php?action=edit&uid_pub='.$myrow["uid_pub"].'&'.$SLINE.'">'.get_img("infedit.gif",20,20,$text_info["c_einfo"],get_js_action(6)).'</a>';
	if ($myrow["isenable"]) {
	...
	$isxmlfeed_enable = ($myrow["isxmlfeed_enable"]) ? $text_info["f_Active"] : $text_info["f_Disable"];

//	$all_status_str .= ($myrow["xmlstatus"]) ? $text_info["f_Active"] : $text_info["f_Disable"];
	$str_link_chstatus = '<a href="publishers.php?action=chstatus&elist[]='.$myrow["uid_pub"].'&'.$SLINE.'" onClick="return window.confirm(\''.$text_info["i_ch_user_status"].'\')">'.$led_onoff.'</a>';
	$str_link_del = '<a href="publishers.php?action=delete&elist[]='.$myrow["uid_pub"].'&'.$SLINE.'" onClick="return window.confirm(\''.$text_info["i_delete_user"].'\')">'.get_img("reject.gif",20,20,$text_info["c_delete"],get_js_action(2)).'</a>';
	...
	$DataBody[$num] = array(
		...
		array("tdw"=>"85", "tdclass"=>"tbl_td_data","tdalign"=>"","data"=>'&nbsp;'.$isxmlfeed_enable),
...
//publishers page
$smarty->assign("curpage","publishers");
...
smarty_create_standard_pagenavigation(true,$text_info["n_pages"],$start,$page_count,"publishers.php",$text_info["n_pages_from"],"",$text_info["n_go"],5);
...
array("link"=>$statuslnk,"text"=>$text_info["n_chstatus"],"title"=>$text_info["c_ch_publishers_status"],
...
//Create help button
smarty_create_helpbutton("publishers.html");

//Create hidden values for form
$smarty->assign("fdata_action","publishers.php");
...
save_session_values("publishers");
